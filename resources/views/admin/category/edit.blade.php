...
<div class="max-w-6xl mx-auto bg-white rounded-[40px] p-10 shadow-sm border border-teal-50">
    <div class="flex justify-between items-center mb-10">
        <h3 class="text-2xl font-heading font-bold text-primary italic">Edit <span class="text-secondary">Category</span></h3>
    ...
    <form action="{{ route('admin.category.update', $category) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        <div>
        ...
        <div class="pt-4 text-right">
            <button type="submit" class="bg-primary hover:bg-secondary text-white text-xs font-bold uppercase tracking-widest px-8 py-3 rounded-full transition-colors inline-flex items-center gap-2">
                Update Category <i data-lucide="check" class="w-4 h-4"></i>
            </button>
        </div>
    </form>
</div>
