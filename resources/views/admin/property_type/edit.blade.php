...
<div class="bg-white rounded-[40px] p-10 shadow-sm border border-teal-50">
    <div class="flex justify-between items-center mb-10">
        <h3 class="text-2xl font-heading font-bold text-primary italic">Edit <span class="text-secondary">Type</span></h3>
    ...
    <form action="{{ route('admin.property-type.update', $propertyType) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select name="property_id" required class="w-full bg-teal-50/30 border border-teal-100 rounded-2xl px-4 py-3 text-sm text-primary focus:outline-none focus:border-secondary focus:ring-1 focus:ring-secondary transition-all">
                ...
                @foreach($properties as $property)
                    <option value="{{ $property->id }}" {{ old('property_id', $propertyType->property_id) == $property->id ? 'selected' : '' }}>
                        {{ $property->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $propertyType->name) }}" required class="w-full bg-teal-50/30 border border-teal-100 rounded-2xl px-4 py-3 text-sm text-primary focus:outline-none focus:border-secondary focus:ring-1 focus:ring-secondary transition-all">
        </div>
        ...
        <div class="pt-4 flex justify-end">
            <button type="submit" class="bg-primary hover:bg-secondary text-white text-xs font-bold uppercase tracking-widest px-8 py-4 rounded-full transition-colors flex items-center gap-2">
                <i data-lucide="save" class="w-4 h-4"></i> Update Type
            </button>
        </div>
    </form>
</div>
